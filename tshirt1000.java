...
public class tshirt1000 extends JFrame{
    String value,materials="1000";
    JFrame frame;
    ImageIcon img1,img2;
    JLabel imgl1,amt1,mat1,imgl2,amt2,mat2,size1,size2,s1,s2;
    JButton imgb1,imgb2,whole,end;
    float amount=0;
    tshirt1000(String UserNameLogin){
        value=UserNameLogin;
        value+="\n---------------------------";
        frame=new JFrame("T-Shirts Collection");
        frame.setLayout(null);
        img1=new ImageIcon(getClass().getResource("kidstshirt1.jpg"));
        img2=new ImageIcon(getClass().getResource("tshirt2.jpg"));
        ...
        try{
            ...
            ResultSet rs=stmt.executeQuery("SELECT * FROM boyscasual where Material='"+m1+"'");
            if (rs.next()){
            ...
            rs=stmt.executeQuery("SELECT * FROM boyscasual where Material='"+m2+"'");
            if (rs.next()){
                String a=rs.getString("Amount");
                String s=rs.getString("Size");
                String st=rs.getString("Stocks");
            ...
            public void actionPerformed(ActionEvent e){
               try{
                   ...
                   Class.forName("com.mysql.jdbc.Driver");
                    ...
                    Connection conn=DriverManager.getConnection(connectString,"root","");
                    Statement stmt=conn.createStatement();
                    stmt.execute("UPDATE BoysCasual set Stocks=Stocks-1 where Stocks>=0 AND Material='"+l+"'");
                    ResultSet rs=stmt.executeQuery("SELECT * FROM boyscasual where Material='"+l+"'");
                    if(rs.next()){
